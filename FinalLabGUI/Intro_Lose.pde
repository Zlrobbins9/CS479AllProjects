...
void introDraw()
{
  ...
  textAlign(CENTER);
   ...
   if(sensorConfigured){
     text("Press T to begin the game!", width/2, height/2);
...
void loseDraw(){
  ...
  textSize(100);
  if(playY >= height){
    text("You got pushed back too far!", width/2, 4*height/10);
    
  }else if(fallTimer >= 300){
    text("You lost your balance!", width/2, 4*height/10);
    
  }else if(stressTimer >= 300){
    text("You had a heart attack and died!", width/2, 4*height/10);
